...
if (sortSwitcher) {
    ...
    function bgs2BuildColorRadarSVG(clusters) {
        ...
        // putya: "да, вот так и надо, сделай на основном так же" — про диаграмму тестового сайта:
        // самый лёгкий кластер прижимается к центру и становится центральной вершиной, через
        // которую идёт обводка. Бейдж с процентом у него при этом остаётся снаружи, на своей оси —
        // это осознанно: процент читается на общем кольце подписей, а в фигуре тот же кластер
        // показан своей малой долей.
        const centerPt = pts.length >= 2 ? pts.reduce((a, b) => (b.weight < a.weight ? b : a)) : null;
        if (centerPt) {
            centerPt.r = 0;
            centerPt.x = cx;
            centerPt.y = cy;
            centerPt.isCenter = true;
        }
        ...
        // Когда центральной вершины нет (единственный цвет — прижимать его к центру значило бы
        // оставить пустую диаграмму), центр всё равно должен попасть в обводку, если лежит вне
        // фигуры: это так ровно тогда, когда наибольший угловой разрыв между соседними вершинами
        // не меньше 180°.
        ...
        const centerIsVertex = !centerPt && n > 0 && maxGap >= 180;
        ...
        // Центр как вершина без своего цвета — только в запасном случае выше (один цвет).
