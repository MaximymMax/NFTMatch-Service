(function () {
    ...
    const drilldownSwatch = document.getElementById('cw-drilldown-swatch');
    ...
    // диаграммой слева. Считаем высоту заголовка drilldown и отдаём остаток под список моделей;
    // на мобильном (одна колонка) сброс на CSS-фиксированную.
    ...
    function syncModelsPanelHeight() {
        ...
        const chrome = drilldownHeader.getBoundingClientRect().height + 10; // gap до #cw-drilldown
    ...
    // модели на этот блок, но уже под эту светлость этого же блока" — refreshDrilldown() не
    ...
    // putya: "убери подходящие фоны, выбор коллекций... вместо Оранжевый (30°–60°) выводи просто
    // превью с цветами данного кластера, теперь будет поиск только по всем моделям в этой
    // диаграме" — заголовок стал цветовым превью (drilldownSwatch) вместо текста, блок подходящих
    // фонов и фильтр коллекций убраны целиком.
    async function refreshDrilldown() {
        ...
        drilldownSwatch.style.background = hex;
        ...
        try {
            ...
            const modelsResp = await fetch(`${API_BASE}/GetGlobalColorWheelBucketModels?${params}`);
    ...
    // putya: "рисуй маленькое превью коллекции, базовое" — иконка подарка с того же CDN, что и
    // everywhere на сайте, по id-to-name.json (имя → id), без отдельного бэкенд-запроса; используется
    // превьюшками в дропдаунах вкладки "Похожие" (similarGiftPreviewHtml).
    let giftNameToId = {};
    ...
    // --- Загрузка и отрисовка диаграммы (переиспользуется при смене ползунка светлоты) ---
